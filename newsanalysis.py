...
def view_popular_authors():
    try:
        db, c = connect()
        query = "create or replace view popular_authors as select \
        authors.name, count(articles.author) as views from articles, log, \
        authors where log.path = concat('/article/',articles.slug) and\
